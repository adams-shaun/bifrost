	...
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			if scheme == "Bearer" {
				...
				// credential FORMAT so a VK-only client (opencode,
				// curl, an SDK) can talk to /v1 when DisableAuthOnInference
				// is false.  Falling through to validateSession would
				// 401 here because a VK is not a session token.
